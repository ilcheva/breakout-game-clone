...
export class BallBehavior extends GameObjectBehavior {
    ...
    private createMetalBall() {
        this.gameObjRef.unregisterRenderable('ballImg');
        const texture = GameApplication.getApp().getLoader().resources.balls.textures;
        const ball = new PIXI.Sprite(texture['metalBall']);
        ball.width = 25;
        ball.height = 25;
        ball.anchor.set(0.5);
        ...
        this.gameObjRef.registerRenderable('ball', ball);
    }
    private createBlueBall() {

        this.gameObjRef.unregisterRenderable('ballImg');
        const texture = GameApplication.getApp().getLoader().resources.balls.textures;
        const ball = new PIXI.Sprite(texture['blueBall']);
        ball.width = 25;
        ball.height = 25;
        ball.anchor.set(0.5);
        ...
        this.gameObjRef.registerRenderable('ballImg', ball);
    }
    ...
    private onBrickHit(e: any) {
        ...
        if (e.brickType === BrickType.TYPE_3) {
            ...
            if (this.timeOutId) {
                clearTimeout(this.timeOutId);
                // EventDispatcher.getInstance().getDispatcher().emit(GameEvents.BALL_RESET);
            }
            EventDispatcher.getInstance().getDispatcher().emit(GameEvents.BALL_SPEEDUP);
